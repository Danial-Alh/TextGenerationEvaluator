...
class LeakGAN(object):
    def __init__(self, sequence_length, num_classes, vocab_size,
        ...
        feature_array = tensor_array_ops.TensorArray(dtype=tf.float32, size=self.sequence_length + 1,
                                                     dynamic_size=False, infer_shape=True, clear_after_read=False)
        real_goal_array = tensor_array_ops.TensorArray(dtype=tf.float32,
        ...
        gen_o_worker_array = tensor_array_ops.TensorArray(dtype=tf.float32,
                                                          size=int(self.sequence_length / self.step_size),
                                                          dynamic_size=False, infer_shape=True, clear_after_read=False)
        ...
        def preTrain(i, x_t, g_predictions, h_tm1, input_x, h_tm1_manager, last_goal, real_goal, feature_array,
            ...
            sub_feature = tf.cond(((((i) % step_size) > 0)),
                                  lambda: sub_feature,
                                  lambda: (
                                      tf.cond(i > 0, lambda: sub_feature.write(tf.cast(i / step_size, tf.int32) - 1,
                                                                               tf.subtract(feature,
                                                                                           feature_array.read(
                                                                                               i - step_size))),
                                              lambda: sub_feature)))
            ...
            real_goal_array = tf.cond(((i) % step_size) > 0, lambda: real_goal_array,
                                      lambda: tf.cond((i) // step_size < self.sequence_length // step_size,
                                                      lambda: tf.cond(i > 0,
                                                                      lambda: real_goal_array.write(
                                                                          tf.cast(i / step_size, tf.int32),
                                                                          real_sub_goal),
