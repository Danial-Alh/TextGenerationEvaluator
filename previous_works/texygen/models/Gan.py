...
class GeneralGenerator(object):
    ...
    def unbiased_temperature_init(self):
        ...
        def get_expected_foraward_prbability(prefix_len, x_start, h_start):
            def _g_recurrence_1(i, x_t, h_tm1, ln_prob):
                h_t = self.g_recurrent_unit(x_t, h_tm1)  # hidden_memory_tuple
                o_t = self.g_output_unit(h_t)
                next_token = tf.cast(tf.reshape(tf.multinomial(o_t, 1), [self.fork_degree]), tf.int32)
                x_tp1 = tf.nn.embedding_lookup(self.g_embeddings, next_token)  # batch x emb_dim
        ...
        def get_powered_probs(logits, prefix_len, h_start):
            ...
            new_logits = self.unbiased_temperature * logits * tf.expand_dims(log_powered_forward_probs, axis=0)
        ...
        # ####################################
        ta_emb_x = tensor_array_ops.TensorArray(
            dtype=tf.float32, size=self.sequence_length, infer_shape=True, dynamic_size=False)
        ta_emb_x = ta_emb_x.unstack(tf.concat((tf.expand_dims(tf.nn.embedding_lookup(self.g_embeddings,
                                                                                     self.start_token)[:1], axis=0),
        ...
        self.unbiased_temperature_persample_len_ll = \
            tensor_array_ops.TensorArray(dtype=tf.float32, size=self.sequence_length,
                                         element_shape=(self.num_vocabulary,), dynamic_size=False)
        ...
        def _nll_occurance(i, h_tm1):
            ...
            o_t = self.g_output_unit(h_t)  # batch x vocab , logits not prob
            o_t, prob_t = get_powered_probs(o_t, i, h_t)
            self.unbiased_temperature_persample_len_ll.write(i, tf.nn.log_softmax(o_t[0]))
            return i + 1, h_t
        ...
        self.unbiased_temperature_persample_len_ll = \
            tf.reduce_sum(
                tf.one_hot(tf.to_int32(tf.reshape(self.dynamic_batch_x, [-1])), self.num_vocabulary, 1.0, 0.0) * \
    ...
    def unbiased_temperature_generate(self, sess, temperature):
        samples = [sess.run(self.unbiased_temperature_gen_x, {self.unbiased_temperature: temperature})[0]
