...
SAVE_PATH = os.path.abspath(os.path.dirname(os.path.abspath(inspect.getfile(inspect.currentframe())))) + '/save/'
...
def prepare_data_for_cnn(seqs_x, maxlen, filter_h=5):
...
def prepare_data_for_rnn(seqs_x, maxlen):
    lengths_x = [len(s) for s in seqs_x]
...
def calu_cost(f_cost, prepare_data_for_cnn, prepare_data_for_rnn, val, val_lab, kf, maxlen):
    # validation cost
    total_negll = 0.
    total_lab = 0.
    total_cr = 0.
    ...
    for _, train_index in kf:
        ...
        x = prepare_data_for_cnn(sents, maxlen)
        y, y_mask = prepare_data_for_rnn(sents, maxlen)
        ## modified
        lab = [val_lab[t] for t in train_index]
        ##
        negll, cost_lab, cost_cr, cost_cf = f_cost(x, y, y_mask, lab)
        negll, cost_lab, cost_cr, cost_cf = negll * np.sum(y_mask), cost_lab * np.sum(y_mask), cost_cr * np.sum(
            y_mask), cost_cf * np.sum(y_mask)
...
def calu_cost_gan(f_cost, prepare_data_for_cnn, prepare_data_for_rnn, val, val_z, kf, maxlen):
    # validation cost
    total_negll = 0.
    total_gan = 0.
    total_len = 0.
...
# original lr: 0.001
def train_model(train, val, test, train_lab, val_lab, test_lab, ixtoword, n_words=22153, period=887, img_w=300,
                img_h=148, feature_maps=300, maxlen=26,
                filter_hs=[3, 4, 5], n_x=300, n_h=500, n_h2_d=200, n_h2=900, p_lambda_q=0, p_lambda_fm=0.001,
                p_lambda_recon=0.001, n_codes=2, max_epochs=16, lr_d=0.0001, lr_g=0.00005, kde_sigma=1., batch_size=256,
                valid_batch_size=256,
                dim_mmd=32, dispFreq=10, dg_ratio=1, Large=1e3, validFreq=500, saveFreq=500, saveto=SAVE_PATH):
    ...
    use_noise, use_noise2, x, z, d_cost, g_cost, r_cost, fake_recon, acc_fake_xx, acc_real_xx, acc_fake_mean, \
    acc_real_mean, wtf1, wtf2, wtf3, wtf4, wtf5, wtf6, KDE, KDE_input = build_model(
        ...
        for eidx in range(max_epochs):
            ...
            for _, train_index in kf:
                ...
                if np.mod(uidx, dispFreq) == 0:
                    ...
                    print('real ' + str(round(temp_out[0], 2)) + ' fake ' + str(
                        round(temp_out[1], 2)) + ' Covariance loss ' + str(round(temp_out[3], 2)) + ' mean loss ' + str(
                        round(temp_out[5], 2)))
                    print('cost_g ' + str(cost_g) + ' cost_d ' + str(cost_d))
                    print("Generated:" + " ".join([ixtoword[x] for x in temp_out[2][0] if x != 0]))

                    logger.info(
                        'Epoch {} Update {} Cost G {} Real {} Fake {} loss_cov {}  meanMSE {}'.format(eidx, uidx,
                                                                                                      cost_g,
                                                                                                      round(temp_out[0],
                                                                                                            2),
                                                                                                      round(temp_out[1],
