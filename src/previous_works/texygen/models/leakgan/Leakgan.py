...
def generate_samples_gen(sess, trainable_model, batch_size, generated_num, output_file=None, get_code=True, train=0):
    ...
    for _ in range(int(generated_num / batch_size)):
        generated_samples.extend(trainable_model.generate(sess, 1.0))
...
class Leakgan(Gan):
    ...
    def init_metric(self):
        ...
        docsim = DocEmbSim(oracle_file=self.oracle_file, generator_fileb=self.generator_file,
    ...
    def train_oracle(self):
        ...
        for a in range(1):
            g = self.sess.run(self.generator.gen_x, feed_dict={
                              self.generator.drop_out: 1, self.generator.train: 1})
        ...
        self.reward = Reward(model=self.generator, dis=self.discriminator,
                             sess=self.sess, rollout_num=4)
