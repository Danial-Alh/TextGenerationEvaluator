...
def get_ngrams(sentences, n):
    def f(x): return (list(ngrams(x, n)) if len(x) >= n else [])
...
class MultisetDistances(BaseMetric):
    ...
    def get_jaccard_score(self, sentences):
        ...
        ngrams_intersection, ngrams_union, ngrams_abs_diff, ngrams_added = self.get_ngram_stuff(
            sentences)
    ...
    def get_score(self, metric_name, sentences, *args, **kwargs):
