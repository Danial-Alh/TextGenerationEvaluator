...
class RealWorldEvaluator(Evaluator):
    ...
    def init_metrics(self, mode):
        if mode == 'train':
            ...
            self.multiset_distances_rfull = MultisetDistances(test_sentences, min_n=2, max_n=10,
    ...
    def get_test_scores(self, samples):
        ...
        selfbleu_result = SelfBleu(generated_sentences, 2, 10, self.parser, parse=False)\
            .get_score()[1]
        revbleu_result = ReverseBleu(test_sentences, generated_sentences, 2, 10, self.parser, parse=False)\
            .get_score()[1]
